...
def render_content(menu_access: MenuAccess, **kwargs):
    if not menu_access.has_access('工作经历-页面'):
    ...
    return fac.AntdFlex(
        [
            ...
            # 详情弹窗
            fac.AntdModal(
                id='wor-detail-modal', title='员工详情', width=700, maskClosable=False,
                children=[
                    fac.AntdDescriptions(id='wor-detail-descriptions', bordered=True, column=2, layout='vertical')]
            )
        ],
...
# ---------------------- 回调 ----------------------
@callback(Output('wor-info-table', 'data'), Input('wor-search-input', 'value'))
...
@callback(
    [Output('wor-detail-modal', 'visible'), Output('wor-detail-descriptions', 'items')],
    [Input('wor-info-table', 'nClicksButton'), Input('wor-info-table', 'recentlyButtonClickedRow')],
