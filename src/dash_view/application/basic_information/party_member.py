...
def render_content(menu_access: MenuAccess, **kwargs):
    if not menu_access.has_access('党员基本信息-页面'):
    ...
    return fac.AntdFlex(
        [
            ...
            # 详情弹窗
            fac.AntdModal(
                id='par-detail-modal', title='员工详情', width=700, maskClosable=False,
                children=[
                    fac.AntdDescriptions(id='par-detail-descriptions', bordered=True, column=2, layout='vertical')]
            )
        ],
...
# ---------------------- 回调 ----------------------
@callback(Output('par-info-table', 'data'), Input('par-search-input', 'value'))
...
@callback(
    [Output('par-detail-modal', 'visible'), Output('par-detail-descriptions', 'items')],
    [Input('par-info-table', 'nClicksButton'), Input('par-info-table', 'recentlyButtonClickedRow')],
